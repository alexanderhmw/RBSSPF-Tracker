...
public:
    ...
    ROSSub<sensor_msgs::PointCloud2ConstPtr> * vscansub;
    ROSTFSub * tfsub;
    QList< QPair<double,std::vector<double> > > scanlist;
    QList< QPair<double,sensor_msgs::PointCloud2ConstPtr> > vscanlist;
    ...
    LaserScan loglaser;
public:
    ROSPub<visualization_msgs::Marker> * markerpub;
    ROSPub<visualization_msgs::Marker> * rectanglepub;
    ROSPub<visualization_msgs::Marker> * cubicpub;
public:
    bool initSSPF=1;
    int pnum;
    ObjectState particle[RQPN];
    ObjectState average;
    bool pfflag;
    int beamnum;
    int beamid[MAXBEAMNUM];
public:
    QFile file;
public slots:
    void slotReceive();
    void slotReceiveVScan();
    void slotReceiveTF();
    void slotShowScan();
    void slotStart(double x, double y, double theta);
    void slotShowRect(int id);
    void slotMeasure(QPointF * corners);
